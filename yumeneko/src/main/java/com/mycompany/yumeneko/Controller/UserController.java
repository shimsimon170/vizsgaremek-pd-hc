...
public class UserController {
    ...
    @Path("getUserById")
    @Consumes(MediaType.APPLICATION_JSON)
    public Response getUserById(@HeaderParam("token") String jwt, @QueryParam("customerId") Integer userId){
        int isValid = JWT.validateJWT(jwt);
    ...
    @Path("getUserById2")
    @Consumes(MediaType.APPLICATION_JSON)
    public Response getUserById(@HeaderParam("token") String jwt){
        int isValid = JWT.validateJWT(jwt);

        if(isValid == 1){
            JSONObject obj = layer.getUserById(JWT.getUserIdByToken(jwt));
            return Response.status(obj.getInt("statusCode")).entity(obj.toString()).type(MediaType.APPLICATION_JSON).build();
        } else if(isValid == 2){
            return Response.status(400).entity("InvalidToken").type(MediaType.APPLICATION_JSON).build();
        } else {
